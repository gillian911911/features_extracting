...
def stHarmonic(frame, fs):
    ...
    Gamma = numpy.zeros((M), dtype=numpy.int64)
...
def beatExtraction(st_features, win_len, PLOT=False):
    ...
    # max_beat_time = int(round(60 / win_len))
    # max_beat_time =1 if max_beat_time <=1 else max_beat_time #new added
    #changed
    print("win_len", win_len)
    print("max_beat_time", max_beat_time)
    hist_all = numpy.zeros((max_beat_time,))
    for ii, i in enumerate(toWatch):  # for each feature
        DifThres = 2.0 * (
            numpy.abs(st_features[i, 0:-1] - st_features[i, 1::])).mean()  # dif threshold (3 x Mean of Difs)
        if DifThres <= 0:
        ...
        for j in range(len(pos1) - 1):
            ...
            print("max_beat_time",max_beat_time,"posDifs", posDifs)

        [hist_times, HistEdges] = numpy.histogram(posDifs, numpy.arange(0.5, max_beat_time + 1.5))#这一步生成的hist_time = 0


        hist_centers = (HistEdges[0:-1] + HistEdges[1::]) / 2.0
        hist_times = hist_times.astype(float) / st_features.shape[1]
        print("hist_times",hist_times,"st_features.shape[1]",st_features.shape[1],st_features.shape[1],"st_features.shape[1]" )
        hist_all += hist_times
        if PLOT:
    ...
    print("I",I)
    bpms = 60 / (hist_centers * win_len) #每分钟节拍数
    BPM = bpms[I]
    # ... and the beat ratio:
    print("hist_all[I]",hist_all[I] )
